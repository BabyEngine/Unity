    ...
    public class SocketIO {
        ...
        public bool IsAlive {
            get {
                ...
                return socketConnection.IsAlive;
            }
        }
        ...
        /// <param name="data">The data to be serialized/sent</param>
        public void EmitACK(string ev, Action<ACKEvent> ackHandler, params object[] data) {
            ...
            lock (synchronizationQueueLock) {
                synchronizationQueue.Enqueue(() => {
                    ...
                    if (ackHandlers.TryGetValue(packet.ID, out handler)) {
                        handler.Handler(new ACKEvent() { data = data, packet = packet});
        ...
        class AckHandler {
            public Action<ACKEvent> Handler { get; private set; }
            public DateTime Timestamp { get; private set; }

            public AckHandler(Action<ACKEvent> handler) {
                Handler = handler;
