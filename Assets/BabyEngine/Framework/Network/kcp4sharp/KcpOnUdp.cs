...
namespace kcp4sharp {
    public abstract class KcpOnUdp : Output {
        ...
        protected LinkedList<ByteBuffer> received;
        protected LinkedList<ByteBuffer> sendList;
        ...
        public KcpOnUdp(int port) {
            ...
            this.received = new LinkedList<ByteBuffer>();
            this.sendList = new LinkedList<ByteBuffer>();
        }
        /// <summary>
        /// 连接到地址
        /// </summary>
        public void Connect(string host, int port) {
            ...
            UnityEngine.Debug.Log($"{host}{port}");
            //mode setting
        ...
        public override void output(ByteBuffer msg, Kcp kcp, Object user) {
        ...
        private void Received(IAsyncResult ar) {
            ...
            try {
                ...
                lock (LOCK) {
                    this.received.AddLast(new ByteBuffer(data));
        ...
        public void Send(ByteBuffer content) {
            lock (this.SEND_LOCK) {
        ...
        public void Update() {
            ...
            lock (LOCK) {
                while (this.received.Count > 0) {
                    ByteBuffer bb = this.received.First.Value;
                    kcp.Input(bb);
            ...
            lock (this.SEND_LOCK) {
                while (this.sendList.Count > 0) {
                    ByteBuffer item = this.sendList.First.Value;
                    this.kcp.Send(item);
         ...
         * 处理收到的消息
         */
        protected abstract void HandleReceive(ByteBuffer bb);
        /// <summary>
        /// 处理异常
        /// </summary>
        /// <param name="ex"></param>
